...
class Normalizer:
    ...
    @classmethod
    def normalize_salary(cls, salary_min: Optional[float], salary_max: Optional[float], raw: str) -> tuple:
    ...
    def normalize_education(cls, raw: str) -> str:
        ...
        if "硕士" in raw or "研究生" in raw: return "硕士"
        if "本科" in raw or "学士" in raw: return "本科"
        if "大专" in raw or "专科" in raw: return "大专"
        if "高中" in raw or "中专" in raw: return "高中"
        if "不限" in raw: return "学历不限"
        return raw
    ...
    def normalize_industry(cls, raw: str) -> str:
        if not raw:
            return ""
        INDUSTRY_MAP = {
            "互联网": "互联网/IT", "计算机": "互联网/IT", "软件": "互联网/IT", "IT": "互联网/IT",
            "通信": "通信/电子", "电子": "通信/电子",
            ...
            "人工智能": "人工智能", "AI": "人工智能",
            "新能源": "新能源",
            "半导体": "半导体/集成电路", "集成电路": "半导体/集成电路", "芯片": "半导体/集成电路",
            "游戏": "游戏/娱乐",
        }
        raw_lower = raw.strip().lower()
