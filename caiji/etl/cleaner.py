    ...
    def clean(cls, records: List[UnifiedJobSchema]) -> List[UnifiedJobSchema]:
        ...
        for rec in records:
            try:
                ...
                if rec.job_title:
                    # Generate minimal description for records without one
                    if not rec.job_description:
                        parts = [rec.job_title, rec.company_name, rec.location,
                                 rec.experience_required or "", rec.education_required or ""]
                        rec.job_description = " | ".join(p for p in parts if p)
                    cleaned.append(rec)
                else:
                    logger.debug(f"Dropping record {rec.record_id}: empty title")
