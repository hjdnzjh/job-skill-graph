...
def _make_recruitment_record(idx: int, now: datetime) -> UnifiedJobSchema:
    ...
    description = desc_template.format(
        title=title, company=company, industry=industry, location=location,
        salary=f"{salary_min:.0f}K-{salary_max:.0f}K",
        skill1=skills[0], skill2=skills[1] if len(skills) > 1 else "N/A",
        ...
        duty1=duties[0], duty2=duties[1], duty3=duties[2],
    ...
    return UnifiedJobSchema(
        ...
        job_title=title, job_title_raw=title,
        company_name=company, company_name_raw=f"{company}有限公司",
        industry=industry, location=location, location_raw=f"{location}市",
...
def _make_enterprise_record(idx: int, now: datetime) -> UnifiedJobSchema:
    ...
    duties = [
        ...
        f"参与{random.choice(['技术方案评审', '代码审查', '项目规划'])}",
    ]
    random.shuffle(duties)
    ...
    return UnifiedJobSchema(
        ...
        job_title=title, job_title_raw=title,
        company_name=company, company_name_raw=f"{company}",
        industry=industry, location=location, location_raw=location,
        job_description=description,
        salary_min=round(random.uniform(10, 40), 1),
...
def _make_policy_record(idx: int, now: datetime) -> UnifiedJobSchema:
    ...
    return UnifiedJobSchema(
        ...
        job_title=title, job_title_raw=title,
        company_name="政府/政策文件", company_name_raw="",
        industry=random.choice(["人工智能", "数字经济", "新能源", "智能制造", "大数据"]),
        location="全国", location_raw="全国",
        job_description=description,
        salary_min=None, salary_max=None,
        experience_required="", education_required="", job_type="",
        skills_required=skills, skills_preferred=[], abilities=abilities,
...
def _make_academic_record(idx: int, now: datetime) -> UnifiedJobSchema:
    ...
    description = (
        ...
        f"本研究采用{random.choice(['自然语言处理', '深度学习', '计量经济学'])}方法，"
        f"系统分析了{skills[0]}、{skills[1]}等相关技能的需求变化趋势。"
        f"研究发现，复合型人才需求显著增长。\n\n"
        f"关键词：{'; '.join(skills[:3])}；就业市场；技能需求"
    )
